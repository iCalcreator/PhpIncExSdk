...
use Kigkonsult\PhpIncExSdk\Dto\PostalAddress as Dto;

class PostalAddress implements JsonInterface
{
    /**
     * Parse json array to populate new PostalAddress
     *
    ...
    public static function parse( array $jsonArray ) : Dto
    {
        ...
        if( isset( $jsonArray[self::PADDRESS] )) {
            $dto->setPAddress( MLStringType::parse( $jsonArray[self::PADDRESS] ));
        }
    ...
    /**
     * Write PostalAddress Dto properties to json array
     *
     * @param Dto $dto
     * @return object
     */
    public static function write( Dto $dto ) : object
    {
        ...
        if( $dto->isPAddressSet()) {
            $jsonArray[self::PADDRESS] = MLStringType::write( $dto->getPAddress());
        }
