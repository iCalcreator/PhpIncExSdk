...
use Kigkonsult\PhpIncExSdk\Dto\AttackPhase as Dto;

class AttackPhase implements JsonInterface
{
    /**
     * Parse json array to populate new AttackPhase
     *
    ...
    public static function parse( array $jsonArray ) : Dto
    {
        ...
        if( isset( $jsonArray[self::ATTACKPHASEID] )) {
            foreach( $jsonArray[self::ATTACKPHASEID] as $sub ) {
                $dto->addAttackPhaseID( $sub );
            }
        }
    ...
    /**
     * Write AttackPhase Dto properties to json array
     *
     * @param Dto $dto
     * @return object
     */
    public static function write( Dto $dto ) : object
    {
        ...
        if( $dto->isAttackPhaseIDSet()) {
            $jsonArray[self::ATTACKPHASEID] = [];
            foreach( $dto->getAttackPhaseID() as $item ) {
                $jsonArray[self::ATTACKPHASEID][] = $item;
            }
        }
        if( $dto->isUrlSet()) {
            $jsonArray[self::URL] = [];
