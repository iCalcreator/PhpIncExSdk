...
use Kigkonsult\PhpIncExSdk\Dto\FileData as Dto;

class FileData implements JsonInterface
{
    /**
     * Parse json array to populate new FileData
     *
    ...
    public static function parse( array $jsonArray ) : Dto
    {
        ...
        if( isset( $jsonArray[self::FILE] )) {
            foreach( $jsonArray[self::FILE] as $subJsonArray ) {
                $dto->addFile( File::parse( $subJsonArray ));
            }
        }
    ...
    /**
     * Write AlFileDataert Dto properties to json array
     *
     * @param Dto $dto
     * @return object
     */
    public static function write( Dto $dto ) : object
    {
        ...
        if( $dto->isFileSet()) {
            $jsonArray[self::FILE] = [];
            foreach( $dto->getFile() as $item ) {
                $jsonArray[self::FILE][] = File::write( $item );
            }
        }
