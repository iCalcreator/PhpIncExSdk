...
use Kigkonsult\PhpIncExSdk\Dto\SystemImpact as Dto;

class SystemImpact implements JsonInterface
{
    /**
     * Parse json array to populate new SystemImpact
     *
    ...
    public static function parse( array $jsonArray ) : Dto
    {
        ...
        if( isset( $jsonArray[self::COMPLESION] )) {
            $dto->setCompletion( $jsonArray[self::COMPLESION] );
        }
    ...
    /**
     * Write SystemImpact Dto properties to json array
     *
     * @param Dto $dto
     * @return object
     */
    public static function write( Dto $dto ) : object
    {
        ...
        if( $dto->isCompletionSet()) {
            $jsonArray[self::COMPLESION] = $dto->getCompletion();
        }
        if( $dto->isTypeSet()) {
