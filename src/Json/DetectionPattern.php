...
use Kigkonsult\PhpIncExSdk\Dto\DetectionPattern as Dto;

class DetectionPattern implements JsonInterface
{
    /**
     * Parse json array to populate new DetectionPattern
     *
    ...
    public static function parse( array $jsonArray ) : Dto
    {
        ...
        if( isset( $jsonArray[self::DETECTIONCONFIGURATION] )) {
            foreach( $jsonArray[self::DETECTIONCONFIGURATION] as $sub) {
                $dto->addDetectionConfiguration( $sub );
            }
        }
    ...
    /**
     * Write DetectionPattern Dto properties to json array
     *
     * @param Dto $dto
     * @return object
     */
    public static function write( Dto $dto ) : object
    {
        ...
        if( $dto->isDetectionConfigurationSet()) {
            $jsonArray[self::DETECTIONCONFIGURATION] = [];
            foreach( $dto->getDetectionConfiguration() as $item ) {
                $jsonArray[self::DETECTIONCONFIGURATION][] = $item;
            }
        }
