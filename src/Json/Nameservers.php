...
class Nameservers implements JsonInterface
{
    /**
     * Parse json array to populate new Nameservers
     *
    ...
    public static function parse( array $jsonArray ) : Dto
    {
        ...
        if( isset( $jsonArray[self::SERVER] )) {
            $dto->setServer( $jsonArray[self::SERVER] );
        }
    ...
    /**
     * Write Nameservers Dto properties to json array
     *
     * @param Dto $dto
     * @return object
     */
    public static function write( Dto $dto ) : object
    {
        ...
        if( $dto->isServerSet()) {
            $jsonArray[self::SERVER] = $dto->getServer();
        }
        if( $dto->isAddressSet()) {
