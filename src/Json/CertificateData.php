...
class CertificateData implements JsonInterface
{
    /**
     * Parse json array to populate new CertificateData
     *
    ...
    public static function parse( array $jsonArray ) : Dto
    {
        ...
        if( isset( $jsonArray[self::CERTIFICATE] )) {
            foreach( $jsonArray[self::CERTIFICATE] as $subJsonArray ) {
                $dto->addCertificate( Certificate::parse( $subJsonArray ));
            }
        }
    ...
    /**
     * Write CertificateData Dto properties to json array
     *
     * @param Dto $dto
     * @return object
     */
    public static function write( Dto $dto ) : object
    {
        ...
        if( $dto->isCertificateSet()) {
            $jsonArray[self::CERTIFICATE] = [];
            foreach( $dto->getCertificate() as $item ) {
                $jsonArray[self::CERTIFICATE][] = Certificate::write( $item );
            }
        }
