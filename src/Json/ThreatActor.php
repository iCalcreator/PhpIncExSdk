...
use Kigkonsult\PhpIncExSdk\Dto\ThreatActor as Dto;

class ThreatActor implements JsonInterface
{
    /**
     * Parse json array to populate new ThreatActor
     *
    ...
    public static function parse( array $jsonArray ) : Dto
    {
        ...
        if( isset( $jsonArray[self::THREATACTORID] )) {
            foreach( $jsonArray[self::THREATACTORID] as $sub ) {
                $dto->addThreatActorID( $sub );
            }
        }
    ...
    /**
     * Write ThreatActor Dto properties to json array
     *
     * @param Dto $dto
     * @return object
     */
    public static function write( Dto $dto ) : object
    {
        ...
        if( $dto->isThreatActorIDSet()) {
            $jsonArray[self::THREATACTORID] = [];
            foreach( $dto->getThreatActorID() as $item ) {
                $jsonArray[self::THREATACTORID][] = $item;
            }
        }
        if( $dto->isUrlSet()) {
            $jsonArray[self::URL] = [];
