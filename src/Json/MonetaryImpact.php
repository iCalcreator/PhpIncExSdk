...
use Kigkonsult\PhpIncExSdk\Dto\MonetaryImpact as Dto;

class MonetaryImpact implements JsonInterface
{
    /**
     * Parse json array to populate new MonetaryImpact
     *
    ...
    public static function parse( array $jsonArray ) : Dto
    {
        ...
        if( isset( $jsonArray[self::CURRENCY] )) {
            $dto->setCurrency( $jsonArray[self::CURRENCY] );
        }
    ...
    /**
     * Write MonetaryImpact Dto properties to json array
     *
     * @param Dto $dto
     * @return object
     */
    public static function write( Dto $dto ) : object
    {
        ...
        if( $dto->isCurrencySet()) {
            $jsonArray[self::CURRENCY] = $dto->getCurrency();
        }
        if( $dto->isValueSet()) {
