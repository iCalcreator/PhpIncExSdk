...
use Kigkonsult\PhpIncExSdk\Dto\BulkObservableFormat as Dto;

class BulkObservableFormat implements JsonInterface
{
    /**
     * Parse json array to populate new BulkObservableFormat
     *
    ...
    public static function parse( array $jsonArray ) : Dto
    {
        ...
        if( isset( $jsonArray[self::HASH] )) {
            $dto->setHash( Hash::parse( $jsonArray[self::HASH] ));
        }
    ...
    /**
     * Write ABulkObservableFormatlert Dto properties to json array
     *
     * @param Dto $dto
     * @return object
     */
    public static function write( Dto $dto ) : object
    {
        ...
        if( $dto->isHashSet()) {
            $jsonArray[self::HASH] = Hash::write( $dto->getHash());
        }
