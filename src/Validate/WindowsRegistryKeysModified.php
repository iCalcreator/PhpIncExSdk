...
class WindowsRegistryKeysModified extends ValidateBase
{
    /**
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        $key    = self::getKey(  $upper, self::KEY );
        if( ! $dto->isRequiredSet()) { // i.e. Key
            $return = false;
            $result[$key] = self::$REQUIRED;
        }
        else {
            foreach( $dto->getKey() as $x => $item ) {
                if( false === Key::check( $item, $result, self::getKeyIxd( $key, $x ))) {
                    $return = false;
                }
            }
        }
