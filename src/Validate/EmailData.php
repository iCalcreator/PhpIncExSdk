...
class EmailData extends ValidateBase
{
    /**
     * EmailData
     *
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        if( $dto->isEmailHeaderFieldSet()) {
            $key    = self::getKey(  $upper, self::EMAILHEADERFIELD );
            foreach( $dto->getEmailHeaderField() as $x => $item ) {
        ...
        if( $dto->isHashDataSet()) {
            ...
            foreach( $dto->getHashData() as $x => $item ) {
                if( false === HashData::check( $item, $result, self::getKeyIxd( $key, $x ))) {
                    $return = false;
                }
            }
        }
