...
class History extends ValidateBase
{
    /**
     * AttackPattern
     *
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        $key    = self::getKey( $upper, self::HISTORYITEM );
        if( ! $dto->isRequiredSet()) { // i.e. HistoryItem
            $return = false;
            $result[$key] = self::$REQUIRED;
        }
        else {
            foreach( $dto->getHistoryItem() as $x => $item ) {
                if( false === HistoryItem::check( $item, $result, self::getKeyIxd( $key, $x ))) {
                    $return = false;
                }
            }
        }
