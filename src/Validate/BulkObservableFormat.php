...
class BulkObservableFormat extends ValidateBase
{
    /**
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        if( ! $dto->isRequiredSet()) {
            ...
            static $IMPLODELIST = [ self::HASH, self::ADDITIONALDATA ];
            $result[$key] = sprintf( self::$ONEREQUIRED, self::implodeList( $IMPLODELIST ));
        }

        if( $dto->isHashSet() &&
            ( false === Hash::check( $dto->getHash(), $result, self::getKey( $upper, self::HASH )))) {
            $return = false;
        }
