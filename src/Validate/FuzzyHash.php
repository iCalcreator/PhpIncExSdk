...
class FuzzyHash extends ValidateBase
{
    /**
     * FuzzyHash
     *
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        $key    = self::getKey(  $upper, self::FUZZYHASHVALUE );
        if( ! $dto->isRequiredSet()) { // i.e. FuzzyHashValue
            $return = false;
            $result[$key] = self::$REQUIRED;
        }
        else {
            foreach( $dto->getFuzzyHashValue() as $x => $item ) {
