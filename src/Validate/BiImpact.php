...
class BiImpact extends ValidateBase
{
    /**
     * BiImpact
     *
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        if( ! $dto->isRequiredSet()) { // i.e. type
            $return = false;
            self::updResultWithReqPropname( $upper, self::TYPE, $result );
        }
        if( ! self::extAttrOk( $dto->getSeverity(), $dto->isExtSeveritySet(), $errExt )) {
            $return = false;
            self::updResultWithErrExtUse( $upper, self::SEVERITY, $errExt, $result );
        }
