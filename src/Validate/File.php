...
class File extends ValidateBase
{
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        if( $dto->isHashDataSet()) {
            $key    = self::getKey(  $upper, self::HASHDATA );
            if( false === HashData::check( $dto->getHashData(), $result, $key )) {
                $return = false;
            }
        }

        if( $dto->isAssociatedSoftwareSet()) {
            $key    = self::getKey(  $upper, self::ASSOCIATEDSOFTWARE );
            if( false === SoftwareType::check( $dto->getAssociatedSoftware(), $result, $key )) {
                $return = false;
            }
        }
        if( $dto->isFilePropertiesSet()) {
            $key    = self::getKey(  $upper, self::FILEPROPERTIES );
            foreach( $dto->getFileProperties() as $x => $item ) {
