...
class FileData extends ValidateBase
{
    /**
     * FileData
     *
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        $key    = self::getKey(  $upper, self::FILE );
        if( ! $dto->isRequiredSet()) { // i.e. File
            $return = false;
            $result[$key] = self::$REQUIRED;
        }
        else {
            foreach( $dto->getFile() as $x => $item ) {
                if( false === File::check( $item, $result, self::getKeyIxd( $key, $x ))) {
                    $return = false;
                }
            }
        }
