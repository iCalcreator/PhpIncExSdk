...
class BulkObservable extends ValidateBase
{
    /**
     * BulkObservable
     *
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        if( ! $dto->isRequiredSet()) { // i.e. BulkObservableList
            $return = false;
            self::updResultWithReqPropname( $upper, self::BULKOBSERVABLELIST, $result );
        }
        ...
        if( $dto->isBulkObservableFormatSet() &&
            ( false === BulkObservableFormat::check(
                $dto->getBulkObservableFormat(),
                $result, self::getKey( $upper, self::BULKOBSERVABLEFORMAT )))) {
            $return = false;
        }
