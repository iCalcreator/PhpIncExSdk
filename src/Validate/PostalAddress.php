...
class PostalAddress extends ValidateBase
{
    /**
     * PostalAddress
     *
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        $key    = self::getKey(  $upper, self::PADDRESS );
        if( ! $dto->isRequiredSet()) { // i.e. PAddress
            $return = false;
            $result[$key] = self::$REQUIRED;
        }
        elseif( false === MLStringType::check( $dto->getPAddress(), $result, $key )) {
            $return = false;
        }
