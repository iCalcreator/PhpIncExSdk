...
class DetectionPattern extends ValidateBase
{
    /**
     * DetectionPattern
     *
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        $appKey = self::getKey( $upper, self::APPLICATION );
        if( ! $dto->isRequiredSet()) {
            $return = false;
            if( ! $dto->isApplicationSet()) {
                $result[$appKey] = self::$REQUIRED;
            }
            if( ! $dto->isDescriptionSet() && ! $dto->isDetectionConfigurationSet()) {
                $key = $upper . self::$PROPLIST;
                static $IMPLODELIST = [ self::DESCRIPTION, self::DETECTIONCONFIGURATION ];
        ...
        if( $dto->isApplicationSet() &&
            ( false === SoftwareType::check( $dto->getApplication(), $result, $appKey ))) {
            $return = false;
        }
