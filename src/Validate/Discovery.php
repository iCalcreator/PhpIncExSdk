...
class Discovery extends ValidateBase
{
    /**
     * Discovery
     *
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        if( ! self::extAttrOk( $dto->getSource(), $dto->isExtSourceSet(), $errExt )) {
            $return = false;
            self::updResultWithErrExtUse( $upper, self::SOURCE, $errExt, $result );
        }
        ...
        if( $dto->isDetectionPatternSet()) {
            $key    = self::getKey( $upper, self::DETECTIONPATTERN );
            foreach( $dto->getDetectionPattern() as $x => $item ) {
                if( false === DetectionPattern::check( $item, $result, self::getKeyIxd( $key, $x ))) {
                    $return = false;
                }
            }
        }
