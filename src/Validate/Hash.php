...
class Hash extends ValidateBase
{
    /**
     * Hash
     *
    ...
    public static function check( Dto $dto, ?array & $result = [], ?string $upper = '' ) : bool
    {
        ...
        if( ! $dto->isRequiredSet()) {
            ...
            if( ! $dto->isDigestMethodSet()) {
                self::updResultWithReqPropname( $upper, self::DIGESTMETHOD, $result );
            }
            if( ! $dto->isDigestValueSet()) {
                self::updResultWithReqPropname( $upper, self::DIGESTVALUE, $result );
            }
        }
