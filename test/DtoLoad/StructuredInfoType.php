...
class StructuredInfoType
{
    /**
     * Use faker to populate new StructuredInfoType
     *
     * @param Dto $dto
    ...
    public static function load( Dto $dto,  ? bool $allowSelfInvokes = true ) : void
    {
        ...
        BasicStructure::load( $dto );

        $max = $faker->boolean() ? 1 : 2;
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = Platform::load( $allowSelfInvokes );
        }
        $dto->setPlatform( $array );

        $max = $faker->boolean() ? 1 : 2;
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = Scoring::load( $allowSelfInvokes );
        }
        $dto->setScoring( $array );
    }
}
