...
class TimeImpact extends DtoLoadBase
{
    /**
     * Use faker to populate new TimeImpact
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $metric = ( 2 < $faker->randomDigitNotNull ) ? $faker->rfc7970TimeImpactMetric() : self::EXT_VALUE;
        $dto->setMetric( $metric );
        if( self::EXT_VALUE === $metric ) {
            $dto->setExtMetric( $faker->word());
        }

        $duration = ( 2 < $faker->randomDigitNotNull ) ? $faker->rfc7970TimeImpactDuration() : self::EXT_VALUE;
