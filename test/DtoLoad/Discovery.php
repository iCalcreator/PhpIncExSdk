...
class Discovery extends DtoLoadBase
{
    /**
     * Use faker to populate new Discovery
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $source = ( 2 < $faker->randomDigitNotNull ) ? $faker->rfc7970DiscoverySource() : self::EXT_VALUE;
        $dto->setSource( $source );
        if( self::EXT_VALUE === $source ) {
            $dto->setExtSource( $faker->word());
        }
        ...
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = DetectionPattern::load( $allowSelfInvokes );
        }
        $dto->setDetectionPattern( $array );
