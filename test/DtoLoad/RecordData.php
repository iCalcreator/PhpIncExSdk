...
class RecordData extends DtoLoadBase
{
    /**
     * Use faker to populate new RecordData
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $dto->setDateTime( $faker->dateTime( 'now', $faker->timezone()));
        ...
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = RecordPattern::load( $allowSelfInvokes );
        }
        $dto->setRecordPattern( $array );
        ...
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = FileData::load( $allowSelfInvokes );
        }
        $dto->setFileData( $array );

        $max = $faker->boolean() ? 1 : 2;
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = WindowsRegistryKeysModified::load( $allowSelfInvokes );
        }
        $dto->setWindowsRegistryKeysModified( $array );

        $max = $faker->boolean() ? 1 : 2;
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = CertificateData::load( $allowSelfInvokes );
        }
        $dto->setCertificateData( $array );
