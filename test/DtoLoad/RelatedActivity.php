...
class RelatedActivity extends DtoLoadBase
{
    /**
     * Use faker to populate new RelatedActivity
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = IncidentID::load( $allowSelfInvokes );
        }
        $dto->setIncidentID( $array );
        ...
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = ThreatActor::load( $allowSelfInvokes );
        }
        $dto->setThreatActor( $array );

        $max = $faker->boolean() ? 1 : 2;
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = Campaign::load( $allowSelfInvokes );
        }
        $dto->setCampaign( $array );

        $max = $faker->boolean() ? 1 : 2;
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = IndicatorID::load( $allowSelfInvokes );
        }
        $dto->setIndicatorID( $array );
