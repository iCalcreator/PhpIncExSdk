...
class IndicatorExpression extends DtoLoadBase
{
    /**
     * Use faker to populate new IndicatorExpression
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $operator = $faker->rfc7970IndicatorExpressionOperator();
        $dto->setOperator( $operator );
        if( 1 === $faker->randomDigitNotNull ) {
            $dto->setExtOperator( $faker->word() );
        }
        ...
        if( $allowSelfInvokes ) {
            ...
            $dto->setIndicatorExpression( $array );
        }

        $array = [];
        for( $x = 0; $x < 19; $x++ ) {
            // try to test all of Observable child classes
            $array[] = Observable::load( $allowSelfInvokes , $x );
        }
        $dto->setObservable( $array );
        ...
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = IndicatorReference::load( $allowSelfInvokes );
        }
        $dto->setIndicatorReference( $array );
