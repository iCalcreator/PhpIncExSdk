...
class PostalAddress extends DtoLoadBase
{
    /**
     * Use faker to populate new PostalAddress
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $type = ( 2 < $faker->randomDigitNotNull ) ? $faker->rfc7970PostalAddressType() : self::EXT_VALUE;
        $dto->setType( $type );
        if( self::EXT_VALUE === $type ) {
            $dto->setExtType( $faker->word());
        }

        $dto->setPAddress( MLStringType::load( $allowSelfInvokes ));
