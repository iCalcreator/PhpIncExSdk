...
class HashData extends DtoLoadBase
{
    /**
     * Use faker to populate new HashData
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $scope = $faker->rfc7970HashDataScope();
        $dto->setScope( $scope );

        $dto->setHashTargetID( $faker->uuid());

        $max = $faker->boolean() ? 1 : 2;
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = Hash::load( $allowSelfInvokes );
        }
        $dto->setHash( $array );
        ...
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = FuzzyHash::load( $allowSelfInvokes );
        }
        $dto->setFuzzyHash( $array);
