...
class RegistryHandle extends DtoLoadBase
{
    /**
     * Use faker to populate new RegistryHandle
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $dto->setHandle( $faker->word());

        $registry = ( 2 < $faker->randomDigitNotNull ) ? $faker->rfc7970RegistryHandleRegistry() : self::EXT_VALUE;
        $dto->setRegistry( $registry );
        if( self::EXT_VALUE === $registry ) {
            $dto->setExtRegistry( $faker->word());
        }
