...
class Telephone extends DtoLoadBase
{
    /**
     * Use faker to populate new class Telephone extends DtoLoadBase
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $type = ( 2 < $faker->randomDigitNotNull ) ? $faker->rfc7970TelephoneType() : self::EXT_VALUE;
        $dto->setType( $type );
        if( self::EXT_VALUE === $type ) {
            $dto->setExtType( $faker->word());
        }

        $dto->setTelephoneNumber( $faker->e164PhoneNumber());
