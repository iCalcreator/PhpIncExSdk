...
class Node extends DtoLoadBase
{
    /**
     * Use faker to populate new Node
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = DomainData::load( $allowSelfInvokes );
        }
        $dto->setDomainData( $array );

        $max = $faker->boolean() ? 1 : 2;
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = Address::load( $allowSelfInvokes );
        }
        $dto->setAddress( $array );

        $dto->setPostalAddress( MLStringType::load( $allowSelfInvokes ));
