...
class BulkObservable extends DtoLoadBase
{
    /**
     * Use faker to populate new BulkObservable
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $type = ( 2 < $faker->randomDigitNotNull ) ? $faker->rfc7970BulkObservableType() : self::EXT_VALUE;
        $dto->setType( $type );
        if( self::EXT_VALUE === $type ) {
            $dto->setExtType( $faker->word());
        }

        $dto->setBulkObservableFormat( BulkObservableFormat::load( $allowSelfInvokes ));

        $dto->setBulkObservableList( implode( PHP_EOL, $faker->words( 4 )));
