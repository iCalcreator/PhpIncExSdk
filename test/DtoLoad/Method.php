...
class Method extends DtoLoadBase
{
    /**
     * Use faker to populate new Method
     *
    ...
    public static function load( ? bool $allowSelfInvokes = true ) : Dto
    {
        ...
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = AttackPattern::load( $allowSelfInvokes );
        }
        $dto->setAttackPattern( $array );

        $max = $faker->boolean() ? 1 : 2;
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = Vulnerability::load( $allowSelfInvokes );
        }
        $dto->setVulnerability( $array );

        $max = $faker->boolean() ? 1 : 2;
        $array = [];
        for( $x = 0; $x < $max; $x++ ) {
            $array[] = Weakness::load( $allowSelfInvokes );
        }
        $dto->setWeakness( $array );
